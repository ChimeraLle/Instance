...
local function GetImage(url)
    ...
    if status == 200 then
        local imageData = table.concat(response)
        local imageDataFile = love.filesystem.newFileData(imageData, "thumbnail.png")
        local image = love.graphics.newImage(imageDataFile)
        results[url] = image
        return image
    elseif status == 307 then
        local imageData = table.concat(response)
        print("a")
        return GetImage(imageData.imageUrl)
    else
        --love.graphics.print(status, 100, 100)
        return nil
    end
end
...
function love.draw()
    for _, object in ipairs(guiobjects) do
        ...
        elseif object.ClassName == "ImageLabel" then
            local url = object.Image
            if url:sub(1, 13) == "rbxassetid://" then
                url = url:sub(14)
            end
            local image = GetImage("https://thumbnails.roblox.com/v1/assets?assetIds="..url.."&size=420x420&format=Png&isCircular=false")
            if image then
                love.graphics.draw(image, PosX, PosY, 0, SizeX / image:getWidth(), SizeY / image:getHeight())
